...
1. Отслеживание клика по кнопкам (мышка ++, клавиатура). 
++ 2. Отрисовка истории расчета.
3. Отрисовка ответа в реальном времени.
++ 4. Отслеживание определенных нажатых кнопок и их дальнейшие действие.{
    ++ 4.2 Логика построение чисел с плавающей точкой. (дробные числа). {
      ++ 4.2.1 Если стоит ноль, то ставиться после нуля запятая и ввод продолжается.
      ++ 4.2.2 Если до ввода стоит знак, то автоматически приписывать 0 и запятую.
      ++ 4.2.3 Учитывать поставленные запятые до и между знаками. (избегать повторов запятых).
    }
    ++ 4.3 Очищение истории после нажатии (ровно). {
    }
  ++ 4.4 Логика постановка скобок {
        ++ 4.4.1 Если поставил скобку, то нужно будет и ее закрыть при повторном нажатии.
        ++ 4.4.2 НЕ ставить открывающую скобку, если не поставлен знак какой нибудь. А то получается так: 2+2(, а это не правильно! Так не выйдет! Появляется ошибка))!.
    }
    4.5
}
6. Логика вычисление. {
  ++ 6.0.1 Обычные вычесление, это сложение и тд, без скобок 
  }
  
  ++ Логика создание массива для калькулятора.
